...
struct paw32xx_config {
  ...
  size_t scroll_snipe_layers_len;
  int32_t *scroll_snipe_layers;
  size_t scroll_horizontal_snipe_layers_len;
  int32_t *scroll_horizontal_snipe_layers;
  int16_t res_cpi;
  int16_t snipe_cpi;
  uint8_t
      snipe_divisor; // Additional precision divisor for snipe mode (default: 2)
  uint8_t scroll_snipe_divisor; // Additional precision divisor for scroll snipe
                                // mode
...
struct paw32xx_data {
  ...
  int16_t current_cpi;
  int16_t scroll_accumulator; // Accumulator for smooth scrolling (reduced from
                              // int32_t)

  // Mode switching state
  enum paw32xx_current_mode current_mode;
  bool mode_toggle_state;
};
